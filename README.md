...
> **Remote-syslog profiles use RAM-only logging.**
> For the templates ending in `-syslog`, `/var/log` is a tmpfs, the
> journal is volatile, and the rsyslog forwarding queue has no disk spool. The
> profiles without `-syslog` instead keep normal durable logs on their VM disks. See
...
| SSH | Public key only, no root, `MaxAuthTries 3`, `LoginGraceTime 30s` | `/etc/ssh/sshd_config.d/99-harden.conf` |
| fail2ban | Aggressive `sshd` jail, escalating 30m bans, nftables actions | `/etc/fail2ban/jail.local` |
| Kernel | Restricted kptr and ptrace, unprivileged BPF off, redirects off, strict `rp_filter`, SYN cookies | `/etc/sysctl.d/20-hardening.conf` |
...
The `-syslog` profiles deliberately use RAM-only local logging. For these
profiles, a clone never writes routine logs to its disk. `/var/log` is a tmpfs
